import { SHOW_ROOM_NUMBERS, SHOW_ROOM_SIZES } from "../config/roomPresentation";
...
const RoomModal: React.FC<RoomModalProps> = ({ isOpen, onClose, onSave, editingRoom }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (SHOW_ROOM_NUMBERS && !formData.roomNumber.trim()) missing.push('roomNumber');
    if (!formData.name.trim()) missing.push('name');
    if (!formData.category) missing.push('category');
    if (!formData.floor) missing.push('floor');
    if (SHOW_ROOM_SIZES && (!formData.size || sizeNum <= 0)) missing.push('size');
    ...
    try {
      ...
      sileo.success({
        title: 'Room saved',
        description: `${formData.name} is now saved in the hotel inventory.`
      });
      onClose();
    } catch (err: any) {
      ...
      <div className="glass-card w-full max-w-5xl max-h-[95vh] sm:max-h-[92vh] flex flex-col rounded-[1.5rem] sm:rounded-[2.5rem] shadow-3xl overflow-hidden border border-white/15 animate-in zoom-in-95 duration-300">
          ...
          <>
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-5 sm:p-10 custom-scrollbar space-y-6 sm:space-y-8">
                ...
                <div className={`grid grid-cols-1 ${SHOW_ROOM_NUMBERS ? "sm:grid-cols-2" : ""} gap-4 sm:gap-8`}>
                    {SHOW_ROOM_NUMBERS && <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center px-1">
                           <label className="text-[9px] sm:text-[10px] text-slate-500 font-black uppercase tracking-widest">Room Name</label>
                ...
                <div className={`grid grid-cols-1 ${SHOW_ROOM_SIZES ? "sm:grid-cols-3" : "sm:grid-cols-2"} gap-4 sm:gap-8`}>
                    <div className="space-y-2">
                        <div className="flex justify-between items-center px-1">
                    ...
                    {SHOW_ROOM_SIZES && <div className="space-y-2">
                        <div className="flex justify-between items-center px-1">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center px-1">
                       <label className="text-[9px] sm:text-[10px] text-slate-500 font-black uppercase tracking-widest">Description (optional)</label>
                    </div>
