...
const Bookings: React.FC = () => {
  ...
  // Handle visual feedback for high-accuracy lookups
  useEffect(() => {
    ...
    if (q.length >= 3) {
      ...
      if (found) {
        if (
          ...
          found.status === BookingStatus.CHECKED_OUT
        ) {
          setLookupError(`Booking is ${found.status}`);
          setLookupResult(null);
        } else {
          setLookupResult(found);
          setLookupError(null);
          // Auto-select if it's a very strong match
          if (
            found.bookingCode.toUpperCase() === q ||
            found.id.toUpperCase() === q
          ) {
        ...
        setLookupResult(null);
        setLookupError(q.length > 6 ? "Dossier not found" : null);
      }
    } else {
      setLookupResult(null);
      setLookupError(null);
    }
  }, [lookupId, bookings, setSelectedBookingId]);
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              <Search
                size={12}
                className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${lookupResult ? "text-brand-500" : "text-slate-500"}`}
              />
              <input
                type="text"
                ...
                className={`w-full bg-white/5 border rounded-md py-2.5 pl-9 pr-3 text-[10px] text-slate-200 outline-none transition-all placeholder:text-slate-600 ${
                  lookupResult
                    ? "border-brand-500/50 bg-brand-500/5 ring-4 ring-brand-500/10 shadow-2xl"
                    : "border-white/5 focus:bg-white/10"
                }`}
              />
            </div>
            <button
          ...
          <div className="glass-card rounded-md flex-1 flex flex-col overflow-hidden shadow-xl border border-white/5">
            <div className="px-4 py-3 border-b border-white/5 flex justify-between items-center bg-slate-900/40">
              ...
              {lookupId && (
                <button
                  onClick={() => setLookupId("")}
                  className="text-[8px] font-black uppercase text-rose-500 flex items-center gap-1 hover:text-rose-400"
                >
                ...
                <tbody className="divide-y divide-white/5">
                  {paginatedBookings.length === 0 ? (
                    <tr>
                      <td
                        colSpan={6}
                        className="py-20 text-center text-slate-700 font-black uppercase tracking-[0.3em] text-[10px]"
                      >
                        No dossiers found matching criteria
                      </td>
                    </tr>
                  ) : (
                        ...
                        >
                          <td className="px-5 py-5">
                            <div className="flex flex-col">
                              <span className="text-[14px] font-black text-white group-hover:text-blue-400 transition-colors leading-none mb-1">
                                {guestName}
                              </span>
                          ...
                          <td className="px-5 py-5">
                            <span
                              className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-dash border ${
                                b.status === BookingStatus.CHECKED_IN
                                  ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30"
                                  : b.status === BookingStatus.CONFIRMED
                                    ? "bg-blue-600/15 text-blue-400 border-blue-600/30"
                                    : b.status === BookingStatus.CHECKED_OUT
                                      ? "bg-slate-500/15 text-slate-500 border-slate-700"
                          ...
                          <td className="px-5 py-5">
                            <span className="text-[13px] font-black text-slate-200">
                              Room {room?.roomNumber || "---"}
                            </span>
                            <p className="text-[8px] text-slate-600 font-bold uppercase mt-0.5">
                              {room?.category}
                            </p>
                          </td>
                          <td className="px-5 py-5">
                            <span className="text-[14px] font-black text-white italic">
          ...
          <div className="glass-card rounded-[2.5rem] p-8 flex flex-col h-full overflow-y-auto custom-scrollbar border border-white/10 bg-[#0a0f1a] relative">
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-[11px] text-[#3b82f6] font-black tracking-[0.2em] uppercase mt-1">
                  REF: {selectedBooking.bookingCode}
                </p>
              </div>
              <button
                onClick={() => setSelectedBookingId(null)}
                className="p-3 bg-white/5 rounded-full text-slate-500 hover:text-rose-500 transition-all border border-white/10"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-6 flex-1">
              <div className="flex flex-col items-center text-center p-8 bg-[#161d2b] rounded-[2rem] border border-white/5 shadow-inner">
                ...
                <h4 className="text-xl font-black text-white italic uppercase tracking-tighter mb-1 truncate w-full">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center bg-[#0d131f] p-6 rounded-2xl border border-white/5">
                  ...
                  <span className="text-[15px] font-black text-[#3b82f6] tracking-tight">
                ...
                <div className="flex justify-between items-center bg-[#0d131f] p-6 rounded-2xl border border-white/5">
                  ...
                  <span className="text-[15px] font-black text-[#10b981] tracking-tight">
