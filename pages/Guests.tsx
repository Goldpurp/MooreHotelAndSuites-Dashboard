...
const Guests: React.FC = () => {
  ...
  const getActiveStay = (guest: Guest) => {
    ...
    const guestEmailLower = String(guest.email || '').toLowerCase();
    
    const booking = (bookings || []).find(b => {
      // 1. Status Filter (Normalization)
      const status = String(b.status || '').toLowerCase().replace(/[-_ ]/g, '');
      const isCheckedIn = status === 'checkedin';
      if (!isCheckedIn) return false;

      // 2. Strict ID Match (Authority)
      const bookingGuestId = String(b.guestId || '').toLowerCase();
      if (bookingGuestId && guestIdLower && bookingGuestId === guestIdLower) return true;

      // 3. Fallback: Email Match (Only if ID is null/missing)
      const bookingEmail = String(b.guestEmail || '').toLowerCase();
      if (!bookingGuestId && bookingEmail && guestEmailLower && bookingEmail === guestEmailLower) return true;
  ...
  const processedGuests = useMemo(() => {
    ...
    // Grouping by unique identifier to ensure no double-showing
    return (guests || [])
      .filter(g => {
