      ...
      <div className="split-main flex flex-col gap-4">
        ...
        <div className="flex border-b border-white/5 gap-6 overflow-x-auto no-scrollbar">
          ...
          <button
            ...
            className={`pb-3 adaptive-text-xs font-black uppercase tracking-widest transition-all relative ${activeTab === "history" ? "text-blue-400" : "text-slate-600"}`}
          >
            History (
            {
            ...
            {activeTab === "history" && (
              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 shadow-blue-500/50"></div>
            )}
          </button>
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {paginatedResidents.length === 0 ? (
                  <tr>
                    ...
                    >
                      No guest records found in this view
                    </td>
                  </tr>
                ) : (
                        ...
                        <td className="responsive-table-padding">
                          <div className="flex items-center gap-4">
                            <div className="relative shrink-0">
                              ...
                              {active && (
                                <div
                                  className={`absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-slate-900 ${isOverdue ? "bg-rose-500 animate-ping" : "bg-emerald-500 animate-pulse"}`}
                                ></div>
                              )}
                            </div>
        ...
        <div className="split-side flex flex-col gap-4 animate-in slide-in-from-right-4 duration-500 h-full overflow-hidden shrink-0">
          <div className="glass-card rounded-2xl p-8 flex flex-col border border-white/10 bg-[#0a0f1a] shadow-2xl h-full overflow-y-auto">
            ...
            {selectedResident.activeStay?.isOverdue && (
              <div className="mb-6 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl animate-pulse flex items-start gap-3">
                <ShieldAlert
                  size={20}
            ...
            <div className="flex flex-col items-center mb-10 pt-4">
              <div className="relative mb-6">
                <img
                  ...
                  className="w-20 h-20 rounded-2xl object-cover ring-2 ring-white/10 shadow-2xl"
                  alt=""
                />
                {selectedResident.activeStay && (
                  <div
                    className={`absolute -bottom-1 -right-1 p-1.5 rounded-lg border-2 border-slate-950 text-white shadow-xl ${selectedResident.activeStay.isOverdue ? "bg-rose-600" : "bg-emerald-600"}`}
                  >
                    {selectedResident.activeStay.isOverdue ? (
                      <Clock size={14} />
                    ) : (
              ...
              <h3 className="adaptive-text-lg font-black text-white uppercase text-center leading-[1] tracking-tighter truncate w-full px-2 mb-2">
              ...
              <div className="px-4 py-1.5 bg-white/5 border border-white/5 rounded-xl flex items-center gap-2">
                ...
                <p className="text-[8px] text-slate-600 font-black uppercase tracking-widest truncate max-w-[120px]">
                  {selectedResident.id}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6 flex-1 pr-1">
              <div className="bg-[#0d131f] p-5 rounded-2xl space-y-4 border border-white/5 shadow-inner">
                <div className="flex items-center gap-4 text-slate-500">
                  ...
                  <span className="adaptive-text-sm font-bold truncate leading-none">
                    {selectedResident.email || "No Email Record"}
                  </span>
                </div>
                <div className="flex items-center gap-4 text-slate-500 pt-4 border-t border-white/5">
                  ...
                  <span className="adaptive-text-sm font-bold leading-none">
                    {selectedResident.phone || "No Phone Record"}
                  </span>
                </div>
              </div>
              ...
              {selectedResident.activeStay && (
                <div
                  className={`p-5 rounded-2xl border space-y-4 shadow-xl ${selectedResident.activeStay.isOverdue ? "bg-rose-500/5 border-rose-500/20" : "bg-emerald-500/5 border-emerald-500/20"}`}
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-2.5 rounded-xl border shrink-0 ${selectedResident.activeStay.isOverdue ? "bg-rose-500/10 border-rose-500/20 text-rose-400" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"}`}
                    >
                      <Bed size={20} />
                    </div>
                    <div className="min-w-0">
                ...
                <div className="space-y-1.5 max-h-[160px] overflow-y-auto">
                  {selectedResident.history.length === 0 ? (
                    ...
                    selectedResident.history.map((stay: Booking) => (
                      <div
                        key={stay.id}
                        className="p-3 bg-white/[0.03] border border-white/5 rounded-xl hover:bg-white/[0.06] transition-all group flex items-center justify-between gap-2"
                      >
                        <div className="min-w-0">
                        ...
                        <button
                          ...
                          className="text-brand-500 hover:text-brand-400 opacity-0 group-hover:opacity-100 transition-all font-black shrink-0"
                        >
