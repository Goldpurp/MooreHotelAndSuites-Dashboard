...
const Dashboard: React.FC = () => {
    ...
    try {
      const [analyticsRes, dailyRes] = await Promise.all([
        api.get("/api/analytics/overview", {
          params: { period: timeFilter.toLowerCase() },
        }),
  ...
  const operationalQueue = useMemo(() => {
    ...
    return [...bookings]
      ...
      .sort((a, b) => {
        ...
        if (
          a.status === BookingStatus.CheckedIn &&
  ...
  const stats = useMemo(() => {
    ...
    const periodBookings = validBookings.filter(
      (b) => now.getTime() - new Date(b.createdAt).getTime() <= filterTime,
    );

    const periodRev = periodBookings.reduce(
      (sum, b) => sum + (b.amount || 0),
      0,
    );
    const occupiedUnits = (rooms || []).filter(
      (r) => r.status === RoomStatus.Occupied,
    ).length;
    const currOcc =
      (rooms || []).length > 0
        ? Math.round((occupiedUnits / rooms.length) * 100)
        : 0;
    ...
    const currADR =
      periodBookings.length > 0
        ? Math.round(periodRev / periodBookings.length)
        : 0;
  ...
  const roomStatusData = useMemo(() => {
    return [
      {
        name: "Occupied",
        value: (rooms || []).filter((r) => r.status === RoomStatus.Occupied)
          .length,
        color: "#3b82f6",
      },
      {
        name: "Available",
        value: (rooms || []).filter((r) => r.status === RoomStatus.Available)
          .length,
        color: "#10b981",
      },
      {
        name: "Cleaning",
        value: (rooms || []).filter((r) => r.status === RoomStatus.Cleaning)
          .length,
        color: "#f59e0b",
      },
      {
        name: "Repair",
        value: (rooms || []).filter((r) => r.status === RoomStatus.Maintenance)
          .length,
        color: "#ef4444",
      },
      {
        name: "Reserved",
        value: (rooms || []).filter((r) => r.status === RoomStatus.Reserved)
          .length,
        color: "#8b5cf6",
      },
    ];
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-1">
            Real-time Enterprise Ledger Protocol
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          growth={stats.occupancy.growth}
          icon={Bed}
          color="bg-emerald-500/10 text-emerald-400"
        />
        <StatCard
          label="Active Residents"
          value={stats.activeGuests.value}
          growth={0}
          icon={UserCheck}
        ...
        <StatCard
          ...
          value={`₦${stats.adr.value.toLocaleString()}`}
          growth={0}
          icon={Activity}
          ...
          <div className="flex justify-between items-start mb-8">
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">
                Inflow trend for current {timeFilter.toLowerCase()}
              </p>
            </div>
          </div>
        ...
        <div className="glass-card p-6 rounded-2xl flex flex-col">
          ...
          <div className="space-y-2 flex-1 overflow-y-auto custom-scrollbar pr-1">
            {roomStatusData.map((item) => (
              <div
                key={item.name}
                className="flex items-center justify-between p-2.5 rounded-xl bg-white/5 border border-white/5 group hover:border-white/10 transition-all"
              >
                <div className="flex items-center gap-3">
                  <div
                  ...
                  ></div>
                  <span className="text-slate-300 text-[11px] font-black uppercase tracking-dash">
                    {item.name}
                  </span>
                </div>
        ...
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-slate-900/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-dash mt-1">
                Most urgent pending actions & arrivals
              </p>
            </div>
          </div>
          <button
            onClick={() => setActiveTab("bookings")}
            className="bg-white/5 hover:bg-white/10 text-slate-300 text-[10px] font-black uppercase tracking-widest px-5 py-2.5 rounded-xl border border-white/5 flex items-center gap-2 transition-all"
          >
            ...
            <tbody className="divide-y divide-white/5">
              {operationalQueue.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                    className="py-20 text-center text-slate-700 font-black uppercase text-[10px] tracking-[0.4em]"
                  >
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-10 h-10 rounded-xl border flex items-center justify-center font-black italic ${
                          ...
                          <div>
                            <p className="text-[14px] font-black text-white group-hover:text-blue-400 transition-colors">
                              {booking.guestFirstName} {booking.guestLastName}
                            </p>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-dash">
                      ...
                      <td className="px-6 py-5">
                        <p
                          className={`text-[12px] font-black ${isToday ? "text-amber-500" : "text-slate-300"}`}
                        >
                      ...
                      <td className="px-6 py-5">
                        <p className="text-[13px] font-black text-slate-300">
                      ...
                      <td className="px-6 py-5">
                        <span
                          className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border flex items-center gap-2 w-fit ${
                            booking.status === BookingStatus.CheckedIn
                        ...
                        >
                          <span
                            className={`w-1 h-1 rounded-full ${booking.status === BookingStatus.CheckedIn ? "bg-emerald-400 animate-pulse" : "bg-current"}`}
                          ></span>
                          {booking.status === BookingStatus.CheckedIn
                            ? "Checked In"
                            : booking.status}
                      ...
                      <td className="px-6 py-5 text-right">
                        ...
                        <p className="text-[9px] text-slate-600 font-black uppercase tracking-dash">
